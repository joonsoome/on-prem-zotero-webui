			...
			return {
				...
				libraries: [
					action.userId && action.libraries.includeMyLibrary && {
						...
						isFileUploadAllowed: allowUploads && (action.apiKey ? true : false),
						isFileUploadAllowedByGroup: false,
						name: 'My Library',
						isExternal: false,
						isPublic: false,
		...
		case RECEIVE_GROUPS:
			var libraries = [
				...
				...action.groups.map(group => ({
					...
					isFileUploadAllowed: state.allowUploads && determineIfGroupAllowsUploads(group, state.userId),
					isFileUploadAllowedByGroup: determineIfGroupAllowsUploads(group, state.userId),
					isExternal: false,
